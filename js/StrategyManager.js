...
class StrategyManager {
   ...
   * @param {Object} db - SQLite database for storing efficiency and trade history of each strategy
   * @param {Array<{name: String, dbLen: Number, f: Function}>} indicators
   */
  constructor(client, db, indicators) {
    this.client = client;
    this.db = db;
  ...
  openPosition(op, strategyId, symbol, price) {
    ...
    if (numOfPos < 3) {
      ...
      DB.saveOrder(this.db, strategyId, symbol, price, op == "BUY");
  ...
  sendEvent(openedTrades, db) {
    ...
    for (const i of this.indicators) {
      ...
      if (i.dbLen > db.length) continue;
      //
      // Cuts part of db which indicator needs
      const part = db.slice(db.length - i.dbLen);
      //
